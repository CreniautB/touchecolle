let list1 =[
  ["Zèbre","hennit"],
  ["Vache","meugle"],
  ["Grenouille","coasse"],
  ["Roitelet","zinzinule"],
  ["Rossignol","chante"],
  ["Sanglier","grommelle"],
  ["Sarcelle","truffle"],
  ["Sauterelle","stridule"],
  ["Hyène","ricane"],
  ["Serpent","siffle"],
  ["Pourceau","grognonne"],
  ["Goéland","raille"],
  ["Rat","piaule"],
  ["Renard","glapit"],
  ["Rhinocéros","barète"],
  ["Poule","caquette"],
  ["Pigeon","caracoule"],
  ["Pie","bavarde"],
  ["Lion","rugit"],
  ["Pintade","criaille"]
]

  let list2 = [
    ["âne","ânesse"],
    ["bouc","chèvre"],
    ["canard","cane"],
    ["cerf","biche"],
    ["chameau","chamelle"],
    ["cheval","jument"],
    ["chevreuil","chevrette"],
    ["coq","poule"],
    ["porc","truie"],
    ["dindon","dinde"],
    ["jars","oie"],
    ["lièvre","hase"],
    ["loup","louve"],
    ["merle","merlette"],
    ["mouton","brebis"],
    ["sanglier","laie"],
    ["singe","guenon"],
    ["tigre","tigresse"],
    ["laie","marcassin"],
    ["biche","faon"]
  ]

  let list3 = [
    ["Azote","N"],
    ["Platine","Pt"],
    ["Arsenic","As"],
    ["Or","Au"],
    ["Brome","Br"],
    ["Carbone","C"],
    ["Calcium","Ca"],
    ["Chlore","Cl"],
    ["Etain","Sn"],
    ["Chrome","Cr"],
    ["Cuivre","Cu"],
    ["Fluor","F"],
    ["Fer","Fe"],
    ["Hydrogène","H"],
    ["Hélium","He"],
    ["Mercure","Hg"],
    ["Iode","I"],
    ["Potassium","K"],
    ["Tungstène","W"],
    ["Sodium","Na"]
  ]

  let list4 = [
    ["126/7",18],
    ["275-240",35],
    ["215/5",43],
    ["380-334",46],
    ["450/30",15],
    ["159/3",53],
    ["330/6",55],
    ["559-496",63],
    ["121/11",11],
    ["294/6",49],
    ["169/13",13],
    ["208/4",52],
    ["232/4",58],
    ["459-397",62],
    ["259-190",69],
    ["550-480",70],
    ["203/7",29],
    ["354/6",59],
    ["70/5",14],
    ["600/30",20]
  ]

  let list5 = [
    ["Albanie","Tirana"],
    ["Arménie","Erevan"],
    ["Australie","Canberra"],
    ["Autriche","Vienne"],
    ["Bahamas","Nassau"],
    ["Bahreïn","Manama"],
    ["Bangladesh","Dacca"],
    ["Barbade","Bridgetown"],
    ["Bénin","Porto-Novo"],
    ["Bolivie","Sucre"],
    ["Botswana","Gaborone"],
    ["Bulgarie","Sofia"],
    ["Burundi","Bujumbura"],
    ["Canada","Ottawa"],
    ["Cameroun","Yaoundé"],
    ["Cap-Vert","Praia"],
    ["Comores","Moroni"],
    ["Costa Rica","San José"],
    ["Danemark","Copenhague"],
    ["Dominique","Roseau"]
  ]

  let list6 = [
  ["Finlande","Helsinki"],
  ["Géorgie","Tbilissi"],
  ["Ile Maurice","Port Louis"],
  ["Jamaïque","Kingston"],
  ["Lettonie","Riga"],
  ["Lituanie","Vilnius"],
  ["Maldives","Malé"],
  ["Mauritanie","Nouakchott"],
  ["Namibie","Windhoek"],
  ["Nicaragua","Managua"],
  ["Qatar","Doha"],
  ["Seychelles","Victoria"],
  ["Suède","Stockholm"],
  ["Suisse","Berne"],
  ["Chypre","Nicosie"],
  ["Gambie","Banjul"],
  ["Haïti","Port-au-Prince"],
  ["Indonésie","Jakarta"],
  ["Kenya","Nairobi"],
  ["Zimbabwe","Harare"]
]

  let list7 = [
    ["Ostentatoire","Orgueilleux"],
    ["Sublimer","Magnifier"],
    ["Litanie","Enumération"],
    ["Dubitatif ","Méfiant"],
    ["Corroborer","Crédibiliser"],
    ["Eminence","Supériorité"],
    ["Incurie","Négligence"],
    ["résilience ","résistance"],
    ["Abyssal","Considérable"],
    ["Anathème","Condamnation"],
    ["Pléonasme","Répétition"],
    ["Risée","Moquerie"],
    ["Gabegie","Gaspillage"],
    ["Déliquescence ","Détérioration"],
    ["Fustiger","Réprimander"],
    ["Hérésie","Erreur"],
    ["Euphémisme","Périphrase"],
    ["Offusquer","Offenser"],
    ["Florilège","Choix"],
    ["Hégémonie","Suprématie"]
  ]

  let list8 = [
    ["incommensurable","mesurable"],
    ["haranguer","complimenter"],
    ["véloce","lent"],
    ["Insidieux","franc"],
    ["couard","courageux"],
    ["versatile","constant"],
    ["dépensier","avare"],
    ["alourdir","alléger"],
    ["pérennité","brièveté"],
    ["inopportun","propice"],
    ["indulgence","sévérité"],
    ["gloire","déclin"],
    ["dialogue","aparté"],
    ["docile","dur"],
    ["éphémère","éternel"],
    ["rugueux","lisse"],
    ["silencieux","volubile"],
    ["audacieux","timoré"],
    ["réticence","assurance"],
    ["antonyme","synonyme"]
  ]

  let list9 = [
    ["859-793",66],
    ["156/6",26],
    ["210/7",30],
    ["207/9",23],
    ["170/5",34],
    ["480/12",40],
    ["144/12",12],
    ["114/6",19],
    ["296/8",37],
    ["659-595",64],
    ["132/3",44],
    ["750/15",50],
    ["196/7",28],
    ["336/8",42],
    ["192/8",24],
    ["240/4",60],
    ["255/5",51],
    ["240/5",48],
    ["246/6",41],
    ["1000-932",68]
  ]

  let list10 =[
    ["1000 ml","1 l"],
    ["1 km","1000 m "],
    ["1 cm","10 mm"],
    ["1 q","100 kg"],
    ["60 min","3600 s"],
    ["10 cl","100 ml"],
    ["1 a","100 m 2"],
    ["1 kg","1000 g"],
    ["30 min","1800 sec"],
    ["10 t","10000 kg"],
    ["1 m 3","1000 l"],
    ["3,4 oz","100 ml"],
    ["0,5 kg","500 g"],
    ["1,7 oz","50 ml"],
    ["2,54 cm","1p"],
    ["1 h ","10000 m 2"],
    ["1 lb ","453 g"],
    ["10 ml","1 cl"],
    ["10 l","1000 cl"],
    ["1 g","1000 mg"]
  ]

  
  function sendList(level) {
    if (level ===1) {return list1}
    if (level ===2) {return list2}
    if (level ===3) {return list3}
    if (level ===4) {return list4}
    if (level ===5) {return list5}
    if (level ===6) {return list6}
    if (level ===7) {return list7}
    if (level ===8) {return list8}
    if (level ===9) {return list9}
    if (level ===10) {return list10}

}

export default sendList